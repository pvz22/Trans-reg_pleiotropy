...
dir <- "/Users/petravandezande/Documents/Output/Projects/Pleiotropy/YFPcounts" #Path must be modified to indicate the directory containing the sample spreadsheet
salmondir <- "/Users/petravandezande/Documents/Output/Projects/Pleiotropy/Salmonquants/" #Must be modified to indicate the directory containing the salmon output files
...
for (i in 29:length(samplesvec)) {
  ...
  write.table(readout, paste0(outputdir,"/Sample_",unique(samples[samples$condition == samplesvec[i],"Description"]),".txt"), sep = "\t")
}
readout <- as.data.frame(get("Q"))
write.table(readout, paste0(outputdir,"/Sample_3287.txt"), sep = "\t")
