...
ggplot(data = as.data.frame(permutevec), aes(x = permutevec)) +
  geom_histogram() +
  geom_vline(xintercept = 64, color = "red", size = 3) +
  THEMEMAIN() +
...
trees <- pheatmap(as.matrix(residslim), trace = "none", show_rownames = FALSE, scale = "none")
